...
const getLeaderboard = async () => {
    ...
    const url = `https://lbdata.pgatour.com/2021/r/033/leaderboard.json?userTrackingId=${token}`;
    console.log(url);
...
const getIndividualPenalty = (player, leaderboard) => {
    if(player.status === 'cut'){
        ...
        return leaderboard.cut_penalty * 4;
    } else {
        return null;
    }
}
...
function buildleaderboard(callback, count) {
    ...
    getLeaderboard().then((pgadata) => {
        ...
        const key = 'leaderboards/pga/2021/leaderboard.json';
