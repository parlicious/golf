...
const getLeaderboardOld = async () => {
    const leaderboardResponse = await axios.get("https://statdata-api-prod.pgatour.com/api/clientfile/Field?T_CODE=r&T_NUM=535&YEAR=2021&format=json");
    return leaderboardResponse.data;
}

const getLeaderboard = async () => {
    const response = await axios.get('https://token.prod.feedsecurity.ixsebu.com/js');
    ...
    const url = `https://statdata-api-prod.pgatour.com/api/clientfile/Field?T_CODE=r&T_NUM=100&YEAR=2021&format=json&userTrackingId=${token}`;
    console.log(url);
...
var golfers = null;
var tournament_info = {
    "id": "2021-open-championship",
    "tournament_name": "2021 Open Championship",
    "timestamp": Date.now(),
    "picks_per_tier": {
        "A": 1,
        "B": 2,
...
const buildit = async () => {
    ...
    tournament_info.field.sort((a, b) => {
